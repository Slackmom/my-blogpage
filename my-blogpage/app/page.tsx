    ...
    <div className="min-h-screen">
      ...
      <section className="py-28 px-6 md:px-12 max-w-5xl mx-auto">
        ...
        <div className="mt-8 sm:hidden text-center">
          <Link href="/blog" className="text-sm text-[#9B6EFF] font-medium">All posts &rarr;</Link>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
            {recentSessions.map((session, i) => (
              <Link key={session.slug} href={`/sessions/${session.slug}`} className={`group block${i === 2 ? " hidden md:block" : ""}`}>
                <article className="relative rounded-2xl bg-[#0c0c1e] border border-[rgba(155,110,255,0.1)] hover:border-[rgba(155,110,255,0.35)] card-lift p-6 overflow-hidden h-full">
                  <div className="flex items-center justify-between mb-5">
                    <span className={`px-2.5 py-1 rounded-full text-[11px] font-medium ${getTopicStyle(session.topic)}`}>
                      {session.topic}
                    </span>
                    <span className="text-[11px] text-[#636876]">{formatShortDate(session.date)}</span>
                  </div>
                  <h3
                    className="text-base font-bold leading-snug text-[#F8FAFC] mb-2 group-hover:text-white transition-colors"
                    style={{ fontFamily: "var(--font-space-grotesk)" }}
                  >
                    {session.title}
                  </h3>
                  <p className="text-[11px] text-[#636876] line-clamp-2">{session.summary}</p>
                </article>
              </Link>
            ))}
          </div>
            ...
            <div className="relative rounded-2xl bg-[#0c0c1e] border border-[rgba(217,70,239,0.18)] p-8 md:p-12 overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-end justify-between mb-8">
                  ...
                  <Link href="/mentoring" className="text-sm text-[#636876] hover:text-[#D946EF] transition-colors duration-200 hidden sm:block">
                    All stories &rarr;
                  </Link>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-5">
                  {recentStories.map((story) => (
                    <Link key={story.slug} href={`/mentoring/${story.slug}`} className="group block">
                      <article className="rounded-xl bg-[rgba(217,70,239,0.04)] border border-[rgba(217,70,239,0.12)] hover:border-[rgba(217,70,239,0.35)] card-lift p-6">
                        <div className="flex items-center gap-2.5 mb-3">
                          <span className="text-[11px] text-[#D946EF] font-medium">{story.category}</span>
                          <span className="text-[11px] text-[#636876]">{formatDate(story.date)}</span>
                        </div>
                        <h3
                          className="text-lg font-normal leading-snug text-[#F8FAFC] mb-2 group-hover:text-white transition-colors"
                          style={{ fontFamily: "var(--font-dm-serif)" }}
                        >
                          {story.title}
                        </h3>
                        <p className="text-[#636876] text-sm leading-relaxed line-clamp-2">
                          {story.summary}
                        </p>
                      </article>
                    </Link>
                ...
                <div className="mt-6 sm:hidden text-center">
                  <Link href="/mentoring" className="text-sm text-[#D946EF] font-medium">All stories &rarr;</Link>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 px-6 md:px-12 border-t border-[rgba(155,110,255,0.07)]">
        <div className="max-w-4xl mx-auto">
          <div className="relative rounded-3xl bg-[#0c0c1e] border border-[rgba(155,110,255,0.18)] p-8 md:p-20 text-center overflow-hidden">
